...
import { CATEGORIES } from '../utils/constants'; // COLORS buradan silindi
...
export default function ReportsScreen() {
  ...
  useEffect(() => {
    loadData();
  }, [theme]); // Tema değişince grafikler güncellensin diye theme eklendi
  ...
  const processWeeklyData = (sessions) => {
    ...
    const chartData = Object.values(last7DaysMap).map((day) => ({
      label: day.label,
      value: Math.round(day.duration / 60),
    }));
  ...
  const processCategoryData = (sessions) => {
    ...
    CATEGORIES.forEach((cat) => {
      categoryMap[cat.value] = {
        ...
        duration: 0,
        color: cat.color,
        legendFontColor: theme.dark, // DİNAMİK: Yazı rengi temadan gelecek
        legendFontSize: 12,
      };
    });
    ...
    const pieData = Object.values(categoryMap)
      ...
      .map((cat) => ({
        name: cat.name,
        population: Math.round(cat.duration / 60),
        color: cat.color,
        legendFontColor: theme.dark, // DİNAMİK: Burası da güncellendi
  ...
  return (
    <ScrollView
      ...
      refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={theme.dark} />}
    >
      ...
      <View style={styles.chartContainer}>
        ...
        {weeklyData.length > 0 ? (
          <BarChart
            ...
            yAxisSuffix="dk"
            chartConfig={{
              backgroundColor: theme.primary, // DİNAMİK
              backgroundGradientFrom: theme.primary, // DİNAMİK
              backgroundGradientTo: theme.secondary, // DİNAMİK
              decimalPlaces: 0,
      ...
      <View style={styles.chartContainer}>
        ...
        {categoryData.length > 0 ? (
          <PieChart
            ...
            height={220}
            chartConfig={{
              color: (opacity = 1) => theme.dark, // DİNAMİK
            }}
            accessor="population"
...
// STİLLER DİNAMİK HALE GETİRİLDİ
const createStyles = (theme) => StyleSheet.create({
  ...
  statValue: {
    fontSize: 24,
    fontWeight: 'bold',
    color: theme.primary, // Dinamik
  },
  distractionValue: {
    color: theme.danger, // Dinamik
  },
  chartContainer: {
    backgroundColor: theme.white, // Dinamik
    marginHorizontal: 20,
  ...
  clearButton: {
    backgroundColor: theme.danger, // Dinamik
    marginHorizontal: 20,
    padding: 15,
    borderRadius: 10,
    alignItems: 'center',
  ...
  clearButtonText: {
    color: '#FFFFFF', // Sabit beyaz kalabilir
    fontSize: 14,
    fontWeight: '600',
  },
  footer: {
